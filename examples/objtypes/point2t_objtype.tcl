...
critcl::objtype structure point2t {
    double x
    double y
} -trace on
...
critcl::cproc point2t-new {double x double y} point2t {
    point2t* p = (point2t*) ckalloc (sizeof (point2t));
    p->refCount = 0;
    p->x = x;
    p->y = y;
    return p;
...
critcl::cproc point2t-x {point2t p} double {
    return p->x;
}

critcl::cproc point2t-y {point2t p} double {
    return p->y;
}

# ### ### ### ######### ######### #########
## Ready
package provide point2t_objtype 1
